...
public class EmailSenderService implements IEmailSender {
    ...
    private String buildConfirmationEmailBody(Order order) {
        ...
        html.append(
                "<p style='color:#e8e8e8; margin:10px 0 0 0; font-size:16px;'>Thank you for shopping with AIMS</p>");
        html.append("</div>");
        ...
        html.append(
                "<span style='background-color:#28a745; color:#fff; padding:8px 20px; border-radius:20px; font-size:14px; font-weight:bold;'>");
        html.append("Order #").append(order.getOrderId()).append("</span>");
        html.append("<p style='color:#6c757d; margin:15px 0 0 0; font-size:13px;'>📅 ").append(order.getCreatedDate())
                .append("</p>");
        html.append("</div>");
        ...
        html.append(
                "<h2 style='color:#333; font-size:18px; margin:0 0 20px 0; border-bottom:2px solid #667eea; padding-bottom:10px;'>🛒 Order Items</h2>");
        ...
        for (OrderItem orderItem : order.getOrderItems()) {
            ...
            html.append("<div style='font-weight:bold; color:#333;'>").append(orderItem.getProduct().getTitle())
                    .append("</div>");
            html.append("<div style='color:#888; font-size:13px; margin-top:5px;'>Quantity: ")
        ...
        // Price summary
        html.append(
                "<div style='background-color:#f8f9fa; padding:25px; margin:0 25px 25px 25px; border-radius:10px;'>");
        html.append("<table style='width:100%;'>");
        html.append("<tr><td style='padding:8px 0; color:#666;'>Subtotal:</td>");
        html.append("<td style='text-align:right; padding:8px 0;'>").append(formatCurrency(order.getSubtotal()))
                .append("</td></tr>");
        html.append("<tr><td style='padding:8px 0; color:#666;'>VAT (10%):</td>");
        html.append("<td style='text-align:right; padding:8px 0;'>").append(formatCurrency(order.getVat()))
                .append("</td></tr>");
        html.append("<tr><td style='padding:8px 0; color:#666;'>🚚 Delivery Fee:</td>");
        html.append("<td style='text-align:right; padding:8px 0;'>").append(formatCurrency(order.getDeliveryFee()))
                .append("</td></tr>");
        ...
        html.append(
                "<td style='text-align:right; padding:15px 0 0 0; font-size:20px; font-weight:bold; color:#28a745;'>");
        ...
        if (order.getDeliveryInfo() != null) {
            ...
            html.append(
                    "<h2 style='color:#333; font-size:18px; margin:0 0 15px 0; border-bottom:2px solid #667eea; padding-bottom:10px;'>📍 Delivery Address</h2>");
            html.append(
                    "<div style='background-color:#fff3cd; border-left:4px solid #ffc107; padding:15px; border-radius:0 8px 8px 0;'>");
            html.append("<div style='font-weight:bold; color:#333; font-size:15px;'>")
                    .append(order.getDeliveryInfo().getRecipientName()).append("</div>");
            html.append("<div style='color:#666; margin-top:8px;'>📱 ").append(order.getDeliveryInfo().getPhoneNumber())
                    .append("</div>");
            html.append("<div style='color:#666; margin-top:5px;'>🏠 ").append(order.getDeliveryInfo().getAddress())
                    .append("</div>");
            html.append("<div style='color:#666; margin-top:5px;'>📮 ").append(order.getDeliveryInfo().getProvince())
                    .append("</div>");
            html.append("</div>");
            html.append("</div>");
        }
        ...
        html.append(
                "<p style='color:#fff; margin:0 0 10px 0; font-size:14px;'>🚚 We will notify you when your order is shipped!</p>");
        html.append("<p style='color:#aaa; margin:0; font-size:12px;'>Thank you for shopping with AIMS Store</p>");
    ...
    private String buildUpdateEmailBody(Order order) {
        ...
        html.append(
                "<div style='background-color:#d4edda; border:1px solid #c3e6cb; padding:20px; border-radius:10px; margin-bottom:20px;'>");
        ...
        html.append("<span style='font-size:16px; color:#856404;'>Status: <strong>").append(order.getStatus())
                .append("</strong></span>");
        html.append("</div>");

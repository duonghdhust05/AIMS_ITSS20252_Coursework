 ...
 * Handles saving orders to the database and retrieving the auto-generated
 * order_id.
 */
public class OrderRepository {
    ...
    public int saveOrder(Order order) throws SQLException {
        ...
        try {
            ...
            // [Big Change] Turn off auto-commit to group insert order and order_items into
            // 1 Transaction
            // Ensure Atomicity: Both success or neither save.
            ...
            try (PreparedStatement stmt = conn.prepareStatement(insertOrderSQL)) {
                ...
                if (rs.next()) {
                    ...
                    // [Big Change] If all success, proceed to commit to save permanently
                    // (Durability)
                    conn.commit();
                    return orderId;
                } else {
            ...
            if (conn != null) {
                try {
                    // [Big Change] Rollback if any error occurs at order or order_items, keep
                    // DB unchanged
                    conn.rollback();
            ...
            if (conn != null) {
                try {
                    // Restore default state to Connection Pool
        ...
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
            ...
            if (rs.next()) {
                ...
                try {
                    ...
                    System.err.println(
                            "Warning: DeliveryInfo validation failed for order " + orderId + ": " + e.getMessage());
                }
    ...
    /**
     * Update order_status for Product Manager review actions (approve/reject) or
     ...
     */
    public boolean updateOrderStatusWithCheck(int orderId, String newStatus, String expectedOldStatus,
            String cancelReason) throws SQLException {
        String sql = """
                UPDATE orders
                SET order_status = ?, cancel_reason = ?, updated_at = CURRENT_TIMESTAMP
